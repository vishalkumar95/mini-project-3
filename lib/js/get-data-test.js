...
function getQualifiedPatientsForVaccineTrials() {
  ...
  if (zipcode) {
    query.set("address-postalcode", zipcode);
  }
  else if (state) {
  ...
  client.request("Patient?" + query, {
    ...
    patients.forEach(function(patient) {
      ...
      return Promise.all([patient_conditions]).then(function(data) {
        if (!isResolvedCondition(data)) {
          return;
        }
        var patientInfo = {Name: patient.name[0].given[0] + " " + patient.name[0].family, Gender: patient.gender, Contact: patient.telecom[0].value, Birthdate: patient.birthDate, id:patient.id}
...
document.getElementById('add').addEventListener('click', getQualifiedPatientsForVaccineTrials);
...
function isResolvedCondition(conditions) {
